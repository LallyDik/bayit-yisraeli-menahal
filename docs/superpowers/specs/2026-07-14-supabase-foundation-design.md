...
- **פרויקט פרודקשן נפרד עם אימות מייל דלוק.** ההחלטה לכבות אימות מייל תקפה לפרויקט הפיתוח בלבד.
- **החלפת ה-PAT שנחשף.** ה-token של שרת ה-MCP `supabase-branch` (`sbp_51c8...`) הודפס לתוך שיחה. הוא נותן גישה מלאה לכל הפרויקטים באותו חשבון. יש לבטל ולהנפיק חדש.
